...
def calculate_effect(model, x_fixed_fn_list, tds, save_dir="output"):
    ...
    for x_fixed_fn in x_fixed_fn_list:
        ...
        plot_bar(f"{save_dir}/{model_name}_{fn_name}_cos_sim", x_fixed_cos_sims / test_num)

        plot_hist(f"{save_dir}/{model_name}_{fn_name}_prob_diff", prob_diffs)

        plot_hist(f"{save_dir}/{model_name}_{fn_name}_ce_loss", ces)
...
def plot_trace_result(result, save_dir=None, title=None, xlabel=None, modelname=None):
    ...
    with plt.rc_context(rc={"font.family": "DejaVu Sans"}):
        ...
        heatmap_name = f"{save_dir}/{title}_{comp_key}_{comp_kind}_heatmap.png"
        ...
        attn_diff_name = f"{save_dir}/{title}_{comp_key}_{comp_kind}_attn_diff.png"
        if attn_weight_diff is not None:
            plot_matrix(attn_diff_name, attn_weight_diff, xlabel="layer",ylabel="head")
...
if __name__ == "__main__":
    ...
    # plot_trace_result(result, title=model_name, save_dir="output")
    # max_len = 100
    # test_num = 100
    # input_texts = load_dataset("imdb")['train']['text'][:test_num]
    # collect_trace_dict(model, tokenizer, input_texts, device, max_len=max_len, save_dir="tds")
    # calculate_effect(model, x_fixed_fn_list=[without_a, without_m],tds=f"tds/tds_gpt2_knowns1209.pkl")
